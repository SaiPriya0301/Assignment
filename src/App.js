...
function App() {
  ...
  useEffect(() => {
    const fetchAudioFiles = async () => {
      ...
      if (lastAudioKey) {
        const index = files.findIndex(file => file.id === parseInt(lastAudioKey, 10));
        if (index !== -1) {
    ...
    <div className="flex flex-col h-screen p-4">
      ...
      <div className="overflow-scroll hide-scrollbar h-[500px] mt-2">
        {audioFiles.map((file, index) => (
          <div className='bg-teal-500 my-4 p-2 rounded flex flex-row items-center justify-evenly hover:border-cyan-900 border border-slate-300 cursor-pointer' key={file.id} onClick={() => handleSelectAudio(index)}>
            <p className='w-[70%] items-center font-medium'>Audio {file.name}</p>
